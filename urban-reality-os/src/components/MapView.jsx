...
export default function MapView() {
  ...
  return (
    <>
      ...
      {/* AQI Quick Toggle Button - Positioned after Layers panel */}
      <div
        style={{
          position: "absolute",
          top: 80,
      ...
      >
        <button
          onClick={() => setLayers(prev => ({ ...prev, aqi: !prev.aqi }))}
          style={{
            padding: "10px 16px",
            borderRadius: 10,
            border: "none",
            background: layers.aqi ? "rgba(34, 197, 94, 0.9)" : "rgba(2, 6, 23, 0.9)",
            color: "#fff",
            cursor: "pointer",
            fontSize: 13,
            fontWeight: 600,
            ...
            border: layers.aqi ? "1px solid rgba(34, 197, 94, 0.3)" : "1px solid rgba(255,255,255,0.1)"
          }}
          onMouseEnter={(e) => {
            e.target.style.transform = "translateY(-1px)";
            e.target.style.boxShadow = "0 6px 16px rgba(0,0,0,0.4)";
            e.target.style.background = layers.aqi ? "rgba(34, 197, 94, 1)" : "rgba(2, 6, 23, 0.95)";
          }}
          onMouseLeave={(e) => {
            e.target.style.transform = "translateY(0)";
            e.target.style.boxShadow = "0 4px 12px rgba(0,0,0,0.3)";
            e.target.style.background = layers.aqi ? "rgba(34, 197, 94, 0.9)" : "rgba(2, 6, 23, 0.9)";
          }}
        >
          <span style={{ fontSize: 16 }}>🌬️</span>
          <span>AQI {layers.aqi ? "ON" : "OFF"}</span>
        </button>
      </div>
